...
%%
unmarshall_mti_test() ->
	Msg = marshaller_ascii:unmarshal("0210"),
	"0210" = iso8583_message:get(0, Msg),
	[0] = iso8583_message:get_fields(Msg).

pan_test() ->
	Msg = marshaller_ascii:unmarshal("02004000000000000000165234567890123456"),
	"0200" = iso8583_message:get(0, Msg),
	[0, 2] = iso8583_message:get_fields(Msg),
	"5234567890123456" = iso8583_message:get(2, Msg).
	
field_8_9_10_test() ->
	Msg = marshaller_ascii:unmarshal("130001C0000000000000000000010000000200000003"),
...
fields_11_12_13_14_test() ->
	Msg = marshaller_ascii:unmarshal("0200003C00000000000000123415075520121206"),
...
fields_15_to_20_test() ->
	Msg = marshaller_ascii:unmarshal("02000003F000000000000001000200030004005006"),
...
field_26_test() ->
	Msg = marshaller_ascii:unmarshal("0200000008400000000000106"),
	[0, 21, 26] = iso8583_message:get_fields(Msg),
	"06" = iso8583_message:get(?POS_CAPTURE_CODE, Msg).

field_27_to_33_test() ->
	Msg = marshaller_ascii:unmarshal("02000000003F800000009C00000011D00000022C00000033C00000044035551112345678901"),
	"9" = iso8583_message:get(?AUTHORIZING_ID_RESP_LEN, Msg),
	"C00000011" = iso8583_message:get(?AMOUNT_TRAN_FEE, Msg).

fields_34_35_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000060000000261234123412341234123456789035;1234123412341234=0305101193010877?"),
...
fields_36_37_38_test() ->
	Msg = marshaller_ascii:unmarshal("0200000000001C0000000101234567890Query123456 123   "),
...
field_39_test() ->
	Msg = marshaller_ascii:unmarshal("020000000000020000007 "),
	[0, 39] = iso8583_message:get_fields(Msg),
	"7 " = iso8583_message:get(?RESP_CODE, Msg).

field_40_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000001000000R 1"),
	"R 1" = iso8583_message:get(40, Msg).

field_41_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000000800000Term#1  "),
	"Term#1  " = iso8583_message:get(?CARD_ACCEPTOR_TERMINAL_ID, Msg).

field_42_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000000400000CA ID 123      "),
	"CA ID 123      " = iso8583_message:get(42, Msg).

field_43_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000000200000NAME                                  ZA"),
	"NAME                                  ZA" = iso8583_message:get(?CARD_ACCEPTOR_NAME_LOCATION, Msg).
	
field_44_test() ->
	Msg = marshaller_ascii:unmarshal("0200000000000010000006Foo123"),
	"Foo123" = iso8583_message:get(?ADDITIONAL_RESP_DATA, Msg).

field_45_test() ->
	Msg = marshaller_ascii:unmarshal("0200000000000008000006Foo123"),
	"Foo123" = iso8583_message:get(45, Msg).

field_46_test() ->
	Msg = marshaller_ascii:unmarshal("02004000000000040000195234567890123456789013Hello, world!"),
	[0, 2, 46] = iso8583_message:get_fields(Msg),
	"Hello, world!" = iso8583_message:get(46, Msg).

fields_47_48_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000000030000006Hello!008Goodbye!"),
...
fields_49_50_51_test() ->
	Msg = marshaller_ascii:unmarshal("0200000000000000E000A  B  C  "),
...
field_52_test() ->
	Msg = marshaller_ascii:unmarshal("02000000000000001000FD0001020304057F"),
	[0, 52] = iso8583_message:get_fields(Msg),
	<<253, 0, 1, 2, 3, 4, 5, 127>> = iso8583_message:get(52, Msg).

field_53_test() ->
	Msg = marshaller_ascii:unmarshal("020040000000000008001952345678901234567890000000000000017"),
	[0, 2, 53] = iso8583_message:get_fields(Msg),
	"0000000000000017" = iso8583_message:get(?SECURITY_RELATED_CONTROL_INFO, Msg).

field_54_test() ->
	Msg = marshaller_ascii:unmarshal("02004000000000000400195234567890123456789017Additi0nal Am0unt"),
	[0, 2, 54] = iso8583_message:get_fields(Msg),
	"Additi0nal Am0unt" = iso8583_message:get(?ADDITIONAL_AMOUNTS, Msg).

fields_55_56_57_test() ->
	Msg = marshaller_ascii:unmarshal("02004000000000000380195234567890123456789002A1003B22004C333"),
...
fields_58_to_63_test() ->
	Msg = marshaller_ascii:unmarshal("0200400000000000007E195234567890123456789002A1003B22004C333005D4444006E55555007F666666"),
...
field_64_test() ->
	Msg = marshaller_ascii:unmarshal("0200000000000000000180FF01020304057F"),
	[0, 64] = iso8583_message:get_fields(Msg),
	<<128, 255, 1, 2, 3, 4, 5, 127>> = iso8583_message:get(?MESSAGE_AUTHENTICATION_CODE, Msg).

field_66_test() ->
	Msg = marshaller_ascii:unmarshal("0200800000000000000040000000000000001"),
	[0, 66] = iso8583_message:get_fields(Msg),
	"1" = iso8583_message:get(66, Msg).

fields_67_to_70_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000003C0000000000000001002003004"),
...
fields_71_72_test() ->
	Msg = marshaller_ascii:unmarshal("02008000000000000000030000000000000000010002"),
...
field_73_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000000080000000000000110219"),
	[0, 73] = iso8583_message:get_fields(Msg),
	"110219" = iso8583_message:get(?DATE_ACTION, Msg).

field_78_to_82_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000000007C000000000000000000001000000002200000003330000004444000000055555"),
...
fields_91_to_94_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000000000003C00000000A2255555DDDDDDD"),
...
field_97_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000000000000080000000C0000000000000001"),
	[0, 97] = iso8583_message:get_fields(Msg),
	"C0000000000000001" = iso8583_message:get(?AMOUNT_NET_SETTLE, Msg).

field_98_test() ->
	Msg = marshaller_ascii:unmarshal("020080000000000000000000000040000000Payee                    "),
	[0, ?PAYEE] = iso8583_message:get_fields(Msg),
	"Payee                    " = iso8583_message:get(?PAYEE, Msg).

field_101_test() ->
	Msg = marshaller_ascii:unmarshal("02008000000000000000000000000800000017A loong file name"),
	[0, ?FILE_NAME] = iso8583_message:get_fields(Msg),
	"A loong file name" = iso8583_message:get(?FILE_NAME, Msg).

fields_102_103_104_128_test() ->
	Msg = marshaller_ascii:unmarshal("02008000000000000000000000000700000104ID 1281234567890123456789012345678009txn desc.0000000000000000"),
...
encoding_rules_test() ->
	Msg = marshaller_ascii:unmarshal("020040000000000000000001", ?MODULE),
	"0001" = iso8583_message:get(2, Msg).

custom_marshaller_test() ->
	Msg = marshaller_ascii:unmarshal("0200300000000000000034", ?MODULE),
